...
import css from './BuyingBasket.module.scss'
/* import { toggleBuied } from "../../features/games/gameSlice"; */
...
const BuyingBasket = (/* props: [NintendoGameArticle, SonyGameArticle, XboxGameArticle] */) => {
    ...
    /* var buiedGames = x.filter((a) => a.isGettingBuied); */
    /* const dispatch = useAppDispatch(); */
    /* console.log(buiedGames) */
    /* const [data, setData] = useState(buiedGames) */

    const handleDeletge = () => {
        /* const update = data.filter(x => dispatch(toggleBuied(x.id)))
        setData(update); */
    }
    return (
        <>
            <div className={css.myComponent}>
                <button
                    onClick={() => {
                        // buiedGames.splice(0, buiedGames.length);
                        // buiedGames = null;
                        console.log('test');
                        // setData([])
                        handleDeletge()
                ...
                <h2 className={css.text}>Buying Basket: </h2>
